...
void LCD_WriteText(char* text)
{
	while(*text){
		if (*text == 0x0D || *text == 0x0A){
			LCD_GoTo(0,1); // przejscie do nowej linii
		}
		LCD_WriteData(*text++);
	}
}
...
// nr - zmienna okreúlajπca numer znaku (dostÍpnych jest 8)  od 0 - 7.
// znak[] - okreúla kszta≥t znaku.. przyk≥adowo uint8_t serduszko[] = {4, 10, 17, 23, 23, 17, 31, 0}; definiuje wyglπd serca.
